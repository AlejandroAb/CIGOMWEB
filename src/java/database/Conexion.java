...
public class Conexion {
    ...
    public void compilePreparedGene() {
        ...
        String query2 = "select sp.uniprot_id, uniprot_acc, prot_name,ncbi_node.name,tipo_muestra,eval from gen left join gen_swiss_prot as gsp on gsp.gen_id = gen.gen_id left join swiss_prot as sp on sp.uniprot_id = gsp.uniprot_id left join ncbi_node on sp.ncbi_tax_id = ncbi_node.tax_id inner join genoma on genoma.idgenoma = gen.idgenoma inner join muestra on muestra.idmuestra = metagenoma.idmuestra inner join muestreo on muestreo.idmuestreo = muestra.idmuestreo inner join tipo_muestra on tipo_muestra.idtipomuestra = muestreo.idtipomuestra where gen.gen_id = ? order by eval";
