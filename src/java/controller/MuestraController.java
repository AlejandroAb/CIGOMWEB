    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (session  == null || session.isNew() || session.getAttribute("userObj") == null) {
            ...
            if (userPath.equals("/showMuestra")) {
                String idmuestreo = request.getParameter("idMuestra");
                
                MuestraDAO mDao = new MuestraDAO(transacciones);
                Muestra muestra = mDao.initMuestraObject(Integer.parseInt(idmuestreo));  
                
                
                MuestreoDAO mtreoDao = new MuestreoDAO(transacciones);
                Muestreo muestreo = mtreoDao.initMuestreoFromMuestra(Integer.parseInt(idmuestreo));
                
                
                System.out.println("Iniformacion de mediciones:"+muestreo.getMediciones());
              
                    if (muestra == null) {
                    String url = "/WEB-INF/view/error/error.jsp";
                    request.setAttribute("msg", "Error inicializando Etiqueta: " +idmuestreo + "<br>Por favor comunicarse con el administrador del sistema ");
            ...
            RequestDispatcher view = request.getRequestDispatcher("/WEB-INF/view/muestra.jsp");
            view.forward(request, response);
        }
     }
   }
